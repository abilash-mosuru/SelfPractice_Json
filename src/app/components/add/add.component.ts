...
export class AddComponent implements OnInit{
  formG!:FormGroup;
  constructor(private formBuilder:FormBuilder,private service:StudentService,private route:Router){
  }
  ...
  uniqueValidator(cont:AbstractControl):ValidationErrors | null{
    ...
    let value = JSON.parse(localStorage.getItem('Student') || '{}');
    if(Array.isArray(value)){
      const users = value.map((d:any)=>d.username);
  ...
  addInfo(){
    if(this.formG.valid){
      this.service.addStudent(this.formG.value).subscribe();
      this.formG.reset();
